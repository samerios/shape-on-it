...
<?php
//if the user confirm "end shopping" check if total price more than zero
if (isset($_POST['confirm'])) {
    if ($total > 0) {
        // check if there are 'payment data' for this user
        if (checkPaymentDetails(@$user, @$id, 'trainer')) {
            ...
            foreach ($_SESSION["cart"] as $key => $value) {
                snedOrderFromUser(@$id, $order_id, $value["product_id"], 'trainer', $value["item_quantity"], 'user');
            }
