...
{
<?php $isCard = checkPaymentDetails(@$user, @$id, 'user'); ?>
...
if (isset($_POST['confirm'])) {
    if ($total > 0) {
        ...
        foreach ($_SESSION["cart"] as $key => $value) {
            snedOrderFromUser(@$id, $order_id, $value["product_id"], 'user', $value["item_quantity"], 'user');
        }
...
if (isset($_POST['end'])) {
    if (checkPaymentDetails(@$user, @$id, 'user') == 0) {
